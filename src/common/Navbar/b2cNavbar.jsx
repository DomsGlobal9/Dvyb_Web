  ...
  const Navbar = () => {
    ...
    const handleSubcategoryClick = (subcategoryName) => {
      navigate(`/products?category=${encodeURIComponent(subcategoryName.toLowerCase())}`);
      setShowWomenCategories(false);
      setActiveCategory(null);
      ...
      <header className="relative bg-white sticky top-0 z-50 shadow-md">
        ...
        {searchOpen ? (
          ...
          <>
            ...
            {/* Desktop Navigation */}
          {/* Desktop Navigation */}
       <nav className="flex flex-row lg:flex-row lg:justify-center gap-4 sm:gap-8 md:gap-12 lg:gap-[60px] px-4 sm:px-8 h-[63px] justify-around items-center border-t border-b border-gray-300 py-4 text-sm font-medium text-gray-700 lg:space-y-0 lg:space-x-8 relative">
  <div className="relative -ml-9" ref={dropdownRef}>
    <button
      onClick={toggleWomenCategories}
      className="text-[#0C2D5B] cursor-pointer font-semibold pt-3 text-base sm:text-lg lg:text-xl hover:text-[#3476CF]"
    >
      WOMEN
    </button>
    <div className="pt-[12.5px]">
      {showWomenCategories && (
        <div className="fixed left-0 right-0 lg:mt-0 cursor-pointer bg-gray-100 shadow-md z-50 w-screen">
          <div className="flex justify-center items-center space-x-4 sm:space-x-6 lg:space-x-8 h-[52px] bg-[#F2F8FC] pt-4 pb-3 border-b border-gray-200">
            {Object.keys(womenCategories).map((cat, index) => (
              <div key={index}>
                <button
                  onClick={() => setActiveCategory(cat)}
                  className={`transition text-sm sm:text-base pb-2 ${
                    activeCategory === cat
                      ? "text-[#042B44] font-outfit lg:text-[18px] cursor-pointer"
                      : "hover:text-[#0C2D5B] text-gray-600 font-normal cursor-pointer"
                  }`}
                >
                  {cat}
                </button>
          ...
          {activeCategory && (
            <div className="w-full bg-white py-[16.5px] px-4">
              <div className="max-w-7xl mx-auto flex justify-center gap-4 sm:gap-6 lg:gap-8 flex-wrap">
  ...
  <span className="text-[#595959] -ml-10 font-outfit cursor-pointer font-normal text-sm sm:text-base relative">
    MEN 
    <span className="text-[6px] sm:text-[7px] absolute left-[calc(100%+2px)] top-0 font-outfit bg-[#EEF2F866] px-1 whitespace-nowrap">
      COMING SOON
    </span>
  </span>
  
  <span className="text-[#595959] font-outfit cursor-pointer font-normal text-sm sm:text-base relative">
    KIDS 
    <span className="text-[6px] sm:text-[7px] absolute left-[calc(100%+2px)] top-0 font-outfit bg-[#EEF2F866] px-1 whitespace-nowrap">
      COMING SOON
    </span>
  ...
  <button 
    className="text-sm sm:text-[15px] font-outfit text-[#595959] font-normal hidden lg:block" 
