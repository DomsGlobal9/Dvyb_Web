...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      if (firebaseUser) {
        setUser(firebaseUser);                  // 👈 save firebase user

        // Check if user exists in b2c_users
        let userDoc = await getDoc(doc(db, "b2c_users", firebaseUser.uid));
        if (userDoc.exists() && userDoc.data().role === "B2C") {
          setUserRole("B2C");
        } else {
          // Otherwise check in B2BBulkOrders_users
          userDoc = await getDoc(doc(db, "B2BBulkOrders_users", firebaseUser.uid));
